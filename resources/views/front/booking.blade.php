...
@section('content')
	...
	<main class="max-w-[640px] mx-auto min-h-screen flex flex-col relative has-[#Bottom-nav]:pb-[144px]">
		...
		<div class="flex flex-col gap-3 px-5 mt-[30px]">
			<h2 class="font-semibold text-lg leading-[27px]">Reminder</h2>
			<p class="text-sm leading-[24px] text-[#6E6E70]">Pastikan Kembali Barang Yang Anda Sewa!</p>
		</div>
		...
		<form method="POST" action="{{route('front.booking_save', $product->slug)}}" class="flex flex-col gap-[30px] mt-[30px]">
			...
			<div class="flex items-center justify-between px-5">
				<label for="days" class="font-semibold">Berapa hari sewa?</label>
			...
			<div class="flex flex-col px-5 gap-2">
				<label for="date" class="font-semibold">Tanggal Sewa</label>
				<div
					...
					<button type="button" id="DateTriggerBtn" class="w-full appearance-none text-left"
						onclick="document.getElementById('date').showPicker()">Pilih Tanggal</button>
					<input type="date" name="started_at" id="date" class="absolute -z-10 opacity-0" required />
				</div>
			</div>
			<div class="flex flex-col px-5 gap-2">
				<label for="date" class="font-semibold">Pick Up</label>
			...
			<div id="Bottom-nav"
				...
				<div class="flex items-center justify-between p-5 relative z-10">
					...
					<button type="submit"
						class="rounded-full p-[12px_24px] bg-[#FCCF2F] font-bold w-fit">Booking</button>
				</div>
			</div>
		</form>
	</main>
